...
Decorator.prototype.decorated = function () {
    ...
    this.matchers.filter(methodCall).forEach(function (matcher) {
        ...
        if (typeof that.receiver[methodName] === 'function') {
            var callSpec = {
                thisArg: that.receiver,
                func: that.receiver[methodName],
                numArgs: numberOfArgumentsToCapture(matcher)
            };
            container[methodName] = decorate(that, callSpec);
        }
    });
    ...
    if (typeof this.receiver === 'function') {
        ...
        if (candidates.length === 1) {
            var callSpec = {
                thisArg: null,
                func: this.receiver,
                numArgs: numberOfArgumentsToCapture(candidates[0])
            };
            basement = decorate(this, callSpec);
        }
    }
    return basement;
};

Decorator.prototype.concreteAssert = function (baseAssert, args, message, context, thisArg) {
    var f;
    if (this.eagerEvaluation) {
        args.push(this.buildPowerAssertText(message, context));
        return baseAssert.apply(thisArg, args);
    }
    try {
        args.push(message);
        return baseAssert.apply(thisArg, args);
