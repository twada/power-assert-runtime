...
(function (root, factory) {
    ...
    function enhance (target, config) {
        ...
        function _expr (value, location, content) {
            var captured = events;
            events = [];
            return new PowerAssertContext({value: value, location: location, content: content, events: captured});
        }

        config.targetMethods.oneArg.forEach(function (methodName) {
            if (typeof target[methodName] === 'function') {
                proxy[methodName] = singleArgAssert(bind(target[methodName], target), config);
            }
        });
        config.targetMethods.twoArgs.forEach(function (methodName) {
            if (typeof target[methodName] === 'function') {
                proxy[methodName] = doubleArgsAssert(bind(target[methodName], target), config);
            }
        });
    ...
    function singleArgAssert (baseAssert, config) {
        return function (value, message) {
            ...
            if (value instanceof PowerAssertContext) {
                ...
                baseAssert(context.value, message ? message + ' ' + powerAssertText : powerAssertText);
            } else {
    ...
    function doubleArgsAssert (baseAssert, config) {
        return function (arg1, arg2, message) {
            var context, val1, val2, powerAssertText;
            if ((arg1 instanceof PowerAssertContext) || (arg2 instanceof PowerAssertContext)) {
                if (arg1 instanceof PowerAssertContext) {
                    context = extend({}, arg1.context);
                    val1 = arg1.context.value;
                } else {
                    val1 = arg1;
                }
                if (arg2 instanceof PowerAssertContext) {
                    if (context.events) {
                        context.events = context.events.concat(arg2.context.events);
                    } else {
                        context = extend({}, arg2.context);
                    }
                    val2 = arg2.context.value;
                } else {
                    val2 = arg2;
                }
                powerAssertText = config.formatter.format(context).join(config.lineSeparator);
                baseAssert(val1, val2, message ? message + ' ' + powerAssertText : powerAssertText);
            } else {
                baseAssert(arg1, arg2, message);
            }
        };
    }
