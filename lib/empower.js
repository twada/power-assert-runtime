...
(function (root, factory) {
    ...
    function enhance (target, config) {
        ...
        function singleArgAssert (baseAssert) {
        ...
        function doubleArgsAssert (baseAssert) {
        ...
        config.targetMethods.oneArg.forEach(function (methodName) {
            if (typeof target[methodName] === 'function') {
                proxy[methodName] = singleArgAssert(bind(target[methodName], target));
            }
        });
        config.targetMethods.twoArgs.forEach(function (methodName) {
            if (typeof target[methodName] === 'function') {
                proxy[methodName] = doubleArgsAssert(bind(target[methodName], target));
            }
        });
        proxy._capt = _capt;
        proxy._expr = _expr;
