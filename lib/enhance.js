...
function enhance (target, formatter, config) {
    var eagerEvaluation = !(config.modifyMessageOnFail || config.saveContextOnFail),
        ...
        enhancement = (typeof target === 'function') ? decorateArgs(1, null, target, doPowerAssert) : {},
        events = [];
    ...
    matchers.forEach(function (matcher) {
        ...
        if (methodName && typeof target[methodName] === 'function') {
            enhancement[methodName] = decorateArgs(numberOfArgumentsToCapture(matcher), target, target[methodName], doPowerAssert);
        }
    });
